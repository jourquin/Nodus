 ...
 */
public class IncrementalAssignmentWorker extends AssignmentWorker {
   ...
   */
  public IncrementalAssignmentWorker(WorkQueue queue) {
    super(queue);
  }

  /**
   * Runs a Incremental equilibrium assignment algorithm.
   *
  ...
  boolean doAssignment() {
    ...
    for (int nodeIndex = 0; nodeIndex < virtualNet.getVirtualNodeLists().length; nodeIndex++) {
      ...
      // Get the demand associated to this node for the current group and od class
      demandList =
  ...
  /**
   * Build all the paths starting from the origin node, loading each used link with a portion
   * (quantity load factor) of the demand.
   *
   * @param nodeIndex int
   * @return True on success.
   */
    ...
    while (it.hasNext()) {
      ...
      while (currentNode != beginNode) {
        ...
        if (predecessor == 0) {
          ...
          an.virtualLink.addVolume(groupIndex, demand.getQuantity() * loadFactor);
